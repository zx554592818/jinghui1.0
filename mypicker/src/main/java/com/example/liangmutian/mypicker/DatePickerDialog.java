        ...
        public DatePickerDialog create() {
            ...
            final LoopListener maxDaySyncListener = new LoopListener() {
                ...
                public void onItemSelect(int item) {
                    Log.d("aaa", "maxDaySyncListener执行");
                    Calendar c = Calendar.getInstance();
            ...
            final LoopListener dayLoopListener = new LoopListener() {
                @Override
                public void onItemSelect(int item) {
                    //这里只要改变时间就会执行 可以在这里做同步UI更新操作
                    Log.d("aaa", "dayLoopListener执行");
                    ...
                    if (maxYear != null && maxMonth != null && maxDay != null
                        ...
                        //设置这个之后不管怎么设置时间  日期总不会超过当前天
                        loopDay.setCurrentItem(maxDay - 1);
                    }
                    //这里传入时间得到对应的星期 并设置
                    week.setText(DateUtil.getDayWeek(Integer.parseInt(params.loopYear.getCurrentItemValue()), Integer.parseInt(params.loopMonth.getCurrentItemValue()), Integer.parseInt(params.loopDay.getCurrentItemValue())));
                }
            };
            //添加监听
